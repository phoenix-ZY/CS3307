...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        model.to(device)
        if eppoch == 1:
            bert_train(num_epochs, model, train_dataloader,valid_dataloader, device,optimizer,batch_size, load_path= 'results/proprecessed_model_all{}'.format(eppoch),save_path = 'results/based_on_proprecessed_model_all/model_{}'.format(eppoch))
        else:
            bert_train(num_epochs, model, train_dataloader,valid_dataloader, device,optimizer,batch_size ,load_path = 'results/based_on_proprecessed_model_all/model_{}'.format(eppoch - 1), save_path = 'results/based_on_proprecessed_model_all/model_{}'.format(eppoch))
        save_results(test_dataloader,device = torch.device('cpu'),model_name = 'results/model2')
        bert_test(test_dataloader,device, 'results/based_on_proprecessed_model_all/model_{}'.format(eppoch))
        break